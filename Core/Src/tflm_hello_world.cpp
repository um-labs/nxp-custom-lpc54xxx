...
__attribute__((section(".lv_sdram")))
uint8_t tensor_arena[3000];
...
TfLiteStatus ProfileMemoryAndLatency() {
  ...
  tflite::RecordingMicroAllocator* allocator(
      tflite::RecordingMicroAllocator::Create(tensor_arena, 3000));
  tflite::RecordingMicroInterpreter interpreter(
      tflite::GetModel(hello_world_float), op_resolver, allocator,
...
TfLiteStatus LoadFloatModelAndPerformInference() {
  const tflite::Model* model =
      ::tflite::GetModel(hello_world_float);
...
TfLiteStatus LoadQuantModelAndPerformInference() {
  ...
  const tflite::Model* model =
      ::tflite::GetModel(hello_world_int8);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
